...
def add_technical_features(
    ...
    decouple_from_close: bool = False,
) -> pd.DataFrame:
    # --- defaults
    ...
    # ========== DECOUPLE FROM CLOSE: Transform absolute features to relative ratios ==========
    # This reduces multicolinealidad by converting features highly correlated with close
    # to normalized ratios/distances that preserve information but reduce correlation
    if decouple_from_close:
        LOGGER.info("Decoupling features from close: converting absolute values to normalized ratios")

        # 1. OHLC: Replace high/low with normalized ratios
        if "high" in df.columns and "low" in df.columns:
            # High-Low spread normalized by close
            df["hl_spread_norm"] = (df["high"] - df["low"]) / df["close"].replace(0, np.nan)
            # Close position within H-L range (0=at low, 1=at high)
            df["close_hl_position"] = (df["close"] - df["low"]) / (df["high"] - df["low"] + 1e-9)
            # Remove absolute high/low (redundant)
            df.drop(columns=["high", "low"], inplace=True, errors="ignore")

        # 2. Moving Averages: Replace absolutes with normalized distances from close
        for L in sorted(set(MA_WINDOWS)):
            sma_col = f"sma_{L}"
            if sma_col in df.columns:
                # Distance from close as percentage
                df[f"close_sma{L}_dist"] = (df["close"] - df[sma_col]) / df["close"].replace(0, np.nan)
                # Remove absolute SMA
                df.drop(columns=[sma_col], inplace=True, errors="ignore")

        for L in sorted(set(EMA_WINDOWS)):
            ema_col = f"ema_{L}"
            if ema_col in df.columns:
                # Distance from close as percentage
                df[f"close_ema{L}_dist"] = (df["close"] - df[ema_col]) / df["close"].replace(0, np.nan)
                # Remove absolute EMA
                df.drop(columns=[ema_col], inplace=True, errors="ignore")

        # 3. MA cross ratios (relative positioning between MAs)
        if "sma_5" not in df.columns and "sma_20" not in df.columns:
            # Already processed, create ratio from distances
            if "close_sma5_dist" in df.columns and "close_sma20_dist" in df.columns:
                # Ratio of how close price is to short vs long MA
                df["ma5_ma20_ratio"] = df["close_sma5_dist"] / (df["close_sma20_dist"].replace(0, np.nan) + 1e-9)

        # 4. Bollinger Bands: Replace absolutes with position within bands
        if "bb_up" in df.columns and "bb_dn" in df.columns:
            # Close position within Bollinger Bands (0=at lower, 1=at upper)
            df["bb_position"] = (df["close"] - df["bb_dn"]) / (df["bb_up"] - df["bb_dn"] + 1e-9)
            # Remove absolute bands (bb_m already removed in Quick Win)
            df.drop(columns=["bb_up", "bb_dn", "bb_m"], inplace=True, errors="ignore")
        # bb_width already normalized (keep it)

        # 5. Fibonacci Levels: Keep ONLY dist_* (normalized), remove absolute levels
        for (_t, fval, tag) in all_f_levels:
            if tag in df.columns:
                df.drop(columns=[tag], inplace=True, errors="ignore")
        # dist_fib_* columns already normalized (kept)

        # 6. ATR: Normalize by close (volatility as % of price)
        if "atr_14" in df.columns:
            df["atr_14_norm"] = df["atr_14"] / df["close"].replace(0, np.nan)
            df.drop(columns=["atr_14"], inplace=True, errors="ignore")

        # 7. Volume POC: Already has dist_vp (normalized), remove absolute vp_poc
        if "vp_poc" in df.columns:
            df.drop(columns=["vp_poc"], inplace=True, errors="ignore")

        # 8. log_close: Remove (perfect correlation with close)
        df.drop(columns=["log_close"], inplace=True, errors="ignore")

        # 9. Keep: log_ret_*, rsi_14, raw_vol_*, td_*, bb_width, fib_composite, dist_*
        # These are already normalized or relative features

        LOGGER.info("Decoupling complete. Features now expressed as normalized ratios/distances.")
...
def load_scaler(path: Path, feature_cols: Optional[Sequence[str]] = None) -> Any:
    ...
    if feature_cols is not None:
        try:
            # Pass feature_cols as dict for ensure_scaler_feature_names
            ensure_scaler_feature_names(scaler, {"feature_cols": feature_cols})
        except Exception:
            pass
    return scaler
...
# ==========================================
# Gap Detection and Data Continuity Utilities
# ==========================================

def timeframe_to_seconds(timeframe: str) -> int:
    """
    Convert timeframe string to seconds.
    Examples: '1m' -> 60, '30m' -> 1800, '1h' -> 3600, '4h' -> 14400, '1d' -> 86400

    Args:
        timeframe: String like '1m', '5m', '30m', '1h', '4h', '1d'
    ...
    if not m:
        raise ValueError(f"Invalid timeframe format: {timeframe}. Expected format like '30m', '1h', '1d'")
...
def detect_gaps(
    ...
    """
    Detect gaps in time series data where consecutive timestamps differ by more than expected.

    Args:
        df: DataFrame with timestamp column
        timeframe: Expected timeframe (e.g., '30m', '1h', '1d')
        tolerance_factor: Multiplier for expected delta to consider a gap (default 1.5)
        ts_column: Name of timestamp column (default 'ts')

    Returns:
        DataFrame with columns [ts, gap_duration, expected_duration] for each gap
        Empty DataFrame if no gaps found

    Example:
        >>> df = pd.DataFrame({'ts': [0, 1800, 3600, 7200, 9000]})  # 30m data with gap
        >>> gaps = detect_gaps(df, '30m')
        >>> print(gaps)
           ts  gap_duration  expected_duration
        0  3600          3600                1800
    """
    if df.empty or ts_column not in df.columns:
        return pd.DataFrame(columns=[ts_column, 'gap_duration', 'expected_duration'])

    try:
        expected_delta = timeframe_to_seconds(timeframe)
    except ValueError as e:
        LOGGER.warning(f"timeframe_to_seconds failed: {e}. Using default 60s.")
        expected_delta = 60

    threshold = expected_delta * tolerance_factor

    # Ensure df sorted by ts
    df_sorted = df.sort_values(ts_column).reset_index(drop=True)
    ts_vals = df_sorted[ts_column].values
    ...
    for idx in gap_indices:
        gaps.append({
            ts_column: int(ts_vals[idx + 1]),  # timestamp after gap
            'gap_duration': int(diffs[idx]),
            'expected_duration': int(expected_delta)
        })
...
def validate_continuous_data(
    ...
    """
    Validate that time series data is sufficiently continuous (no excessive gaps).

    Args:
        df: DataFrame with timestamp column
        timeframe: Expected timeframe (e.g., '30m', '1h', '1d')
        max_gap_pct: Maximum allowed gap percentage (default 5%)
        tolerance_factor: Multiplier for expected delta to consider a gap
    ...
    Example:
        >>> is_valid, msg = validate_continuous_data(df, '30m', max_gap_pct=0.05)
        >>> if not is_valid:
    ...
    if ts_column not in df.columns:
        return False, f"Timestamp column '{ts_column}' not found"

    gaps = detect_gaps(df, timeframe, tolerance_factor, ts_column)
    n_gaps = len(gaps)

    if n_gaps == 0:
        return True, f"Data is continuous (0 gaps in {len(df)} rows)"

    gap_pct = (n_gaps / max(1, len(df))) * 100

    if gap_pct > max_gap_pct * 100:
        # Calculate total missing duration
        total_gap_duration = gaps['gap_duration'].sum() - gaps['expected_duration'].sum()
        return False, (
            f"Data has {n_gaps} gaps ({gap_pct:.2f}% of rows, exceeds {max_gap_pct*100:.1f}% threshold). "
            f"Total missing duration: {total_gap_duration}s"
        )

    return True, f"Data quality acceptable ({n_gaps} gaps, {gap_pct:.2f}% of rows)"
