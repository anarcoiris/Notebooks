...
def compute_features_with_fiboevo(df_ohlcv: pd.DataFrame, decouple_from_close: bool = False) -> pd.DataFrame:
    """
    ...
    Args:
        ...
        decouple_from_close: If True, converts absolute features to normalized ratios
                            to reduce multicolinealidad with close
    """
    ...
    feats = fiboevo.add_technical_features(close, high=high, low=low, volume=vol, decouple_from_close=decouple_from_close)
...
def build_and_save_dataset(sqlite_path: str,
                           ...
                           decouple_features: bool = False) -> Dict[str, Any]:
    """
    End-to-end pipeline:
    ...
    df_feats = compute_features_with_fiboevo(df_ohlcv, decouple_from_close=decouple_features)
    ...
    # QUICK WIN: Remove features highly correlated with close (multicolinealidad)
    # These features provide redundant information and cause gradient instability in LSTM
    EXCLUDE_HIGHLY_CORRELATED = {
        "high", "low",  # OHLC: high/low ~0.98-0.99 corr with close
        "log_close",  # Perfect correlation (log transform of close)
        "sma_5", "ema_5",  # Very short MAs: ~0.97-0.99 corr, too noisy
        "bb_m",  # Bollinger middle band is just sma_20 (redundant)
        "bb_up", "bb_dn",  # Bollinger bands: ~0.94-0.97 corr with close
    }

    initial_count = len(feature_cols)
    feature_cols = [c for c in feature_cols if c not in EXCLUDE_HIGHLY_CORRELATED]
    removed_count = initial_count - len(feature_cols)

    if removed_count > 0:
        logger.info("Removed %d highly correlated features to reduce multicolinealidad: %s",
                   removed_count, [c for c in EXCLUDE_HIGHLY_CORRELATED if c in df_clean.columns])
        logger.info("Remaining features: %d", len(feature_cols))

    # Use temporal_split_indexes from fiboevo to prevent data leakage
    # This ensures scaler is fitted ONLY on training rows
    if fiboevo and hasattr(fiboevo, "temporal_split_indexes"):
        ...
        try:
            ...
            logger.info("Temporal split: n_train=%d, n_val=%d, n_test=%d, train_rows_end=%d",
                       n_train_seq, n_val_seq, n_test_seq, train_rows_end)
        except Exception as e:
            logger.warning("temporal_split_indexes failed: %s. Falling back to standard split.", e)
            train_rows_end = None
    else:
        logger.warning("fiboevo.temporal_split_indexes not available. Using standard split (may cause data leakage).")
        train_rows_end = None

    # Fit scaler ONLY on training rows (before creating sequences)
    if StandardScaler is None:
        raise RuntimeError("scikit-learn no instalado; instala scikit-learn para usar scaler.")

    scaler = StandardScaler()
    if train_rows_end is not None:
        # Fit scaler on training rows only (CRITICAL for preventing data leakage)
        scaler.fit(df_clean[feature_cols].iloc[:train_rows_end + 1].values)
        logger.info("Scaler fitted on training rows only (0:%d)", train_rows_end + 1)
    else:
        # Fallback: old behavior (NOT RECOMMENDED - potential data leakage)
        logger.warning("WARNING: Fitting scaler on ALL data - potential data leakage!")
        scaler.fit(df_clean[feature_cols].values)

    # Set feature_names_in_ to prevent alignment issues
    scaler.feature_names_in_ = np.array(feature_cols, dtype=object)
    logger.info("Scaler feature_names_in_ set: %d features", len(feature_cols))

    # Transform ALL data with scaler (fitted only on train)
    df_clean[feature_cols] = scaler.transform(df_clean[feature_cols].values)
    logger.info("All features scaled using training-only scaler")
    ...
    # Split sequences
    if train_rows_end is not None:
        # Use the computed splits from temporal_split_indexes
        Xtr = X_all[:n_train_seq]
        ytr = y_all[:n_train_seq]
        voltr = vol_all[:n_train_seq]

        Xv = X_all[n_train_seq:n_train_seq + n_val_seq]
        yv = y_all[n_train_seq:n_train_seq + n_val_seq]
        volv = vol_all[n_train_seq:n_train_seq + n_val_seq]

        Xtst = X_all[n_train_seq + n_val_seq:]
        ytst = y_all[n_train_seq + n_val_seq:]
        voltst = vol_all[n_train_seq + n_val_seq:]
    else:
    ...
    # Note: Data is already scaled (we scaled df_clean before creating sequences)
    # So X_all, Xtr, Xv, Xtst are already scaled - no need to scale again
    Xtr_s = Xtr
    Xv_s = Xv if Xv.size else Xv
    Xtst_s = Xtst if Xtst.size else Xtst
    ...
    if save_dir:
        ...
        meta = {
            ...
            "train_rows_end": int(train_rows_end) if train_rows_end is not None else None,
            "scaler_fitted_on": "train_only" if train_rows_end is not None else "all_data",
...
def parse_args():
    ...
    p.add_argument("--decouple-features", action="store_true", default=False,
                   help="Transform absolute features (high, low, MAs, BBs) to normalized ratios to reduce multicolinealidad")
    return p.parse_args()
...
def main():
    ...
    decouple = getattr(args, 'decouple_features', False)
