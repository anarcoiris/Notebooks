...
class TradingAppExtended:
    ...
    def _run_forecast(self):
        ...
        for t_i in range(n_traj):
            ...
            traj_prices = []
            for step in range(N):
                # Prepare input using prepare_input_for_model if available
                y_pred = None
                try:
                    # Build dataframe from sequence for prepare_input_for_model
                    if hasattr(fibo, "prepare_input_for_model"):
                        seq_df = pd.DataFrame(seq, columns=feature_cols)
                        # Use prepare_input_for_model (handles scaling and validation)
                        xt = fibo.prepare_input_for_model(
                            seq_df,
                            feature_cols,
                            seq_len,
                            scaler=scaler,
                        ...
                        if scaler is not None:
                            try:
                                ...
                                # scaler may expect 2D - we supply seq shaped
                                X_in = X_in
                        xt = torch.tensor(X_in[None, :, :], dtype=torch.float32)

                    # Torch model path
                    if uses_torch:
                        # xt is already tensor, move to device
    ...
    # ---------------------------
    # dentro de la clase TradingAppExtended

    def get_var_value(self, x):
        """Devuelve el valor de una tkinter Variable (.get()) o el valor tal cual si no es variable."""
        try:
            return x.get() if hasattr(x, "get") else x
        except Exception:
            # protección adicional: si .get() lanza, devolvemos None
            return None

    def _run_audit(self):
        """
        Auditoría: corre comprobaciones en background, pero toma valores UI y copias de DF
        EN EL HILO PRINCIPAL; actualiza widgets sólo mediante self.root.after.
        """
        # 1) Leer de la UI (main thread) y preparar copias de data para el worker:
        try:
            # normalizamos las variables que vienen del UI (IntVar/StringVar/... o literales)
            horizon_val = int(self.get_var_value(getattr(self, "horizon", 1)) or 1)
        except Exception:
            horizon_val = 1

        try:
            seq_len_val = int(self.get_var_value(getattr(self, "seq_len", 32)) or 32)
        except Exception:
            seq_len_val = 32

        try:
            val_frac_val = float(self.get_var_value(getattr(self, "val_frac", 0.2)) or 0.2)
        except Exception:
            val_frac_val = 0.2

        # copiar los dataframes (para evitar accesos concurrentes a self.df_*)
        if getattr(self, "df_features", None) is None:
            # si no hay features, informamos y salimos (no lanzar hilo)
            report = {"timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
                    "checks": [{"name": "df_features_present", "ok": False, "msg": "df_features is None"}],
                    "summary": {}}
                ...
                def _ui_write():
                    try:
                        ...
                        for chk in report["checks"]:
                            tag = "OK" if chk.get("ok", None) else ("WARN" if chk.get("ok", None) is False else "N/A")
                            self.audit_text.insert("end", f"{tag:4} {chk['name']:35} - {chk.get('msg','')}\n")
                        ...
                        pass
                self.root.after(0, _ui_write)
            except Exception:
                pass
            return

        # Hacemos las copias (esto puede ser costoso si DF es grande; es intencional para seguridad)
        try:
            dff_copy = self.df_features.copy()
        except Exception:
            dff_copy = self.df_features  # fallback (riesgo de concurrencia)
        try:
            dfs_copy = self.df_scaled.copy() if getattr(self, "df_scaled", None) is not None else None
        except Exception:
            dfs_copy = None

        # 2) Worker: hace el análisis pesado usando las copias y valores capturados
        def _job(dff, dfs, horizon_val, seq_len_val, val_frac_val):
            report = {"timestamp": time.strftime("%Y-%m-%d %H:%M:%S"), "checks": [], "summary": {}}
            try:
                report["checks"].append({"name": "df_features_present", "ok": True, "msg": f"shape={dff.shape}"})
                ...
                # 2) NaN / rows dropped detection
                if getattr(self, "df_loaded", None) is not None:
                ...
                # 3) Scaler checks (si existe dfs/copia del scaler)
                ...
                # 4) Exact-match leakage: features equal a close.shift(-k)
                # Robust: only compare numeric columns (or convertible-to-numeric). Skip metadata/time columns.
                if "close" in dff.columns:
                    M = max(1, min(5, int(horizon_val)))
                    matches = []
                    # determine candidate columns: skip core metadata/time columns
                    meta_candidates = {c for c in dff.columns if c.lower() in ("timestamp", "ts", "time", "date", "symbol", "created_at", "updated_at")}
                    candidate_cols = [c for c in dff.columns if c != "close" and c not in meta_candidates]

                    for k in range(1, M + 1):
                        shifted = dff["close"].shift(-k)

                        # convert shifted to numeric (coerce non-numeric -> NaN)
                        shifted_num = pd.to_numeric(shifted, errors="coerce").values

                        for col in candidate_cols:
                            try:
                                # convert feature col to numeric (coerce non-numeric -> NaN)
                                a_num = pd.to_numeric(dff[col], errors="coerce").values

                                # valid mask: both finite numbers
                                valid = np.isfinite(a_num) & np.isfinite(shifted_num)
                                if valid.sum() < 10:
                                    continue

                                # compare using allclose on the numeric slices
                                if np.allclose(a_num[valid], shifted_num[valid], atol=1e-6, rtol=1e-6):
                                    matches.append({"feature": col, "shift": k})
                            except Exception:
                                # If any unexpected dtype error occurs, skip this column (do not crash audit)
                                continue
                ...
                corr_flags = []
                if "close" in dff.columns:
                    horizon_corr = max(1, int(horizon_val))
                    future_close = dff["close"].shift(-horizon_corr)
                    for col in dff.select_dtypes(include=[np.number]).columns:
                        if col == "close":
                            continue
                        valid_idx = (~dff[col].isna()) & (~future_close.isna())
                        if valid_idx.sum() < 20:
                            continue
                        c = np.corrcoef(dff[col].values[valid_idx], future_close.values[valid_idx])[0, 1]
                ...
                # 7) Recommended split heuristics
                n_total = len(dff)
                seq_len = max(1, int(seq_len_val))
                val_frac = float(val_frac_val) if val_frac_val is not None else 0.2
                n_sequences = max(0, n_total - seq_len - int(horizon_val) + 1)
                n_val_seq = max(1, int(round(val_frac * n_sequences))) if n_sequences > 0 else 0
                n_train_seq = max(0, n_sequences - n_val_seq)
                train_rows_end = seq_len + n_train_seq - 1 if n_train_seq > 0 else seq_len - 1
                ...
                report["checks"].append({"name": "audit_error", "ok": False, "msg": traceback.format_exc()})
            finally:
                # Guardar informe en la instancia
                self._last_audit_report = report
                ...
                def _update_ui():
                    try:
                        ...
                        # si por cualquier motivo no podemos actualizar widgets, al menos mostramos en consola
                        print("Audit report (fallback):", report)
                try:
                    self.root.after(0, _update_ui)
                except Exception:
                    # si no hay root, intentar directo (riesgoso)
                    try:
                        _update_ui()
                    except Exception:
                        print("Audit report:", report)

        # lanzar thread con argumentos locales (capturados)
        t = threading.Thread(target=lambda: _job(dff_copy, dfs_copy, horizon_val, seq_len_val, val_frac_val), daemon=True)
        t.start()
    ...
    def _get_latest_prediction(self):
        ...
        try:
            ...
            # Use prepare_input_for_model if available (prevents double-scaling)
            try:
                if hasattr(fibo, "prepare_input_for_model"):
                    ...
                    if missing:
                        self._enqueue_log(f"Missing features for prediction: {missing}")
                        return

                    # Prepare input (handles scaling, validation, tensor creation)
                    Xp = fibo.prepare_input_for_model(
                        seq_df,
                        feature_cols,
                        seq_len,
                        scaler=scaler,
                    ...
                    self._enqueue_log(f"Prepared input using prepare_input_for_model: shape={Xp.shape}")
                else:
                    # Fallback: manual sequence building
                    self._enqueue_log("Using fallback manual sequence building")
                    ...
                    # choose last sequence and scale
                    ...
                    Xp = _torch.from_numpy(Xp).float()
            except Exception as e:
                self._enqueue_log(f"Input preparation failed: {e}")
                self._enqueue_log(traceback.format_exc())
                return
            ...
            try:
                ...
                model_ref.eval()
                with _torch.no_grad():
                    # Xp is already a tensor from prepare_input_for_model, or numpy array from fallback
                    if not isinstance(Xp, _torch.Tensor):
    ...
    def _load_model_from_artifacts(self):
        """
        Carga modelo, meta y scaler desde ./artifacts using fiboevo.load_model() and fiboevo.load_scaler().
        ...
        try:
            ...
            # require torch
            if torch is None:
                self._enqueue_log("torch no está instalado; no se puede cargar el modelo (instala torch).")
                return

            # Use fiboevo.load_model() - returns (model, meta) tuple
            try:
                model, meta = fibo.load_model(model_path)
                self._enqueue_log(f"Modelo cargado desde {model_path} usando fibo.load_model()")
            except Exception as e:
                self._enqueue_log(f"Fallo al cargar modelo con fibo.load_model(): {e}")
                self._enqueue_log(traceback.format_exc())
                return
            ...
            try:
                ...
                model.to(device)
                model.eval()
                self._enqueue_log(f"Modelo preparado en device {device}.")
            ...
            # Enrich metadata with feature_cols_used if missing
            if "feature_cols" not in self.model_meta and getattr(self, "feature_cols_used", None):
                self.model_meta["feature_cols"] = list(self.feature_cols_used)

            self._enqueue_log(f"Metadata aplicada: {self.model_meta}")

            # Load scaler with feature validation using new API
            if scaler_path.exists():
                try:
                    if hasattr(fibo, 'load_scaler'):
                        # Use new API with feature validation
                        self.model_scaler = fibo.load_scaler(
                            scaler_path,
                            feature_cols=self.model_meta.get("feature_cols")
                        )
                        self._enqueue_log(f"Scaler cargado con validación de features desde {scaler_path}.")
                    else:
                        # Fallback to joblib
                        if joblib is not None:
                            scaler = joblib.load(str(scaler_path))
                            if hasattr(fibo, 'ensure_scaler_feature_names'):
                                scaler = fibo.ensure_scaler_feature_names(scaler, self.model_meta)
                            self.model_scaler = scaler
                            self._enqueue_log(f"Scaler cargado (fallback) desde {scaler_path}.")
                        else:
                            self._enqueue_log("joblib no disponible; no se puede cargar scaler.pkl.")
                            self.model_scaler = None
                except Exception as e:
                    self._enqueue_log(f"Fallo al cargar scaler: {e}")
                    self._enqueue_log(traceback.format_exc())
                    self.model_scaler = None
            else:
                self.model_scaler = None
