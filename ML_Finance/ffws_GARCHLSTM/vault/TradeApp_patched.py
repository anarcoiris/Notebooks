                ...
                if "close" in dff.columns:
                    ...
                    for k in range(1, M + 1):
                        ...
                        for col in dff.columns:
                            ...
                            if valid.sum() < 10:
                                continue
                            if safe_allclose(a[valid], b[valid], atol=1e-6, rtol=1e-6):
    ...
    def _get_latest_prediction(self):
        ...
        try:
            ...
            if not feature_cols or len(feature_cols) == 0:
                self._enqueue_log("No valid feature columns for prediction")
                return
...
def vprint(msg: str, verbose: bool = False):
    if verbose:
        print(msg)
...
def try_load_scaler(path, verbose=False):
    """Robust scaler loader: tries pickle, joblib, torch, numpy, gzip+pickle."""
    p = Path(path)
    if not p.exists():
        raise FileNotFoundError(f"Scaler file not found: {path}")
    head = detect_header(p)
    vprint(f"Scaler header (hex): {head.hex()}", verbose)
    loaders = []
    loaders.append(("pickle", lambda pth: _pickle.load(open(pth, "rb"))))
    if _joblib is not None:
        loaders.append(("joblib", lambda pth: _joblib.load(str(pth))))
    loaders.append(("torch", lambda pth: _importlib.import_module("torch").load(str(pth), map_location="cpu")))
    loaders.append(("numpy", lambda pth: _np.load(str(pth), allow_pickle=True)))
    loaders.append(("gzip+pickle", lambda pth: _pickle.load(_gzip.open(pth, "rb"))))
    last_exc = None
    for name, fn in loaders:
        try:
            obj = fn(p)
            vprint(f"Loaded scaler via {name}: {type(obj)}", verbose)
            return obj
        except Exception as e:
            last_exc = e
            vprint(f"loader {name} failed: {type(e).__name__}: {e}", verbose)
    with open(p, "rb") as f:
        head = f.read(128)
    raise RuntimeError(f"Could not load scaler with known loaders. Last: {last_exc}. File head: {repr(head[:64])}")

def safe_allclose(a, b, atol=1e-6, rtol=1e-6, equal_nan=False):
    """Compare arrays safely converting to numeric, handling datetimes and NaNs."""
    try:
        a_arr = _pd.to_numeric(_pd.Series(a), errors="coerce").values
        b_arr = _pd.to_numeric(_pd.Series(b), errors="coerce").values
    except Exception:
        # fallback: try numpy conversion with coercion
        try:
            a_arr = _np.array(a, dtype=float)
        except Exception:
            a_arr = _np.asanyarray(a)
        try:
            b_arr = _np.array(b, dtype=float)
        except Exception:
            b_arr = _np.asanyarray(b)
    # align lengths
    minlen = min(len(a_arr), len(b_arr))
    a_arr = a_arr[:minlen]
    b_arr = b_arr[:minlen]
    valid = ~_np.isnan(a_arr) & ~_np.isnan(b_arr)
    if valid.sum() < 1:
        return False
    return _np.allclose(a_arr[valid], b_arr[valid], atol=atol, rtol=rtol, equal_nan=equal_nan)

def load_table_as_df(db_path: str, table: str, timestamp_col: str, prefer_scaled: bool = True, verbose: bool = False):
    """Prefer a features_scaled table if present, otherwise fall back to given table."""
    try:
        conn = _sqlite3.connect(db_path)
    except Exception as e:
        raise RuntimeError(f"Could not open DB {db_path}: {e}")
    if prefer_scaled:
        try:
            cur = conn.cursor()
            cur.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='features_scaled'")
            if cur.fetchone():
                q = f"SELECT * FROM features_scaled ORDER BY {timestamp_col} ASC"
                df = _pd.read_sql_query(q, conn, parse_dates=[timestamp_col])
                if verbose:
                    print("Loaded features_scaled table (preferred)")
                conn.close()
                return df
        except Exception as e:
            vprint(f"Could not load features_scaled: {e}", verbose)
    # fallback to requested table
    from contextlib import closing
    with closing(conn):
        cur = conn.cursor()
        cur.execute("SELECT name FROM sqlite_master WHERE type='table' AND name=?", (table,))
        if cur.fetchone() is None:
            raise RuntimeError(f"Table '{table}' not found in DB '{db_path}'")
    try:
        conn = _sqlite3.connect(db_path)
        try:
            df = _pd.read_sql_query(f"SELECT * FROM {table} ORDER BY {timestamp_col} ASC", conn, parse_dates=[timestamp_col])
        except Exception:
            df = _pd.read_sql_query(f"SELECT * FROM {table} ORDER BY {timestamp_col} ASC", conn)
        conn.close()
    except Exception as e:
        raise RuntimeError(f"Error reading table {table}: {e}")
    if verbose:
        print(f"Loaded table {table} with {len(df)} rows and columns: {list(df.columns)[:20]}")
    return df
